...
export DRIVER_AVAIL="$(wget -qO- https://api.github.com/repos/DanITman/unraid-nvidia-vgpu-driver/releases/tags/${KERNEL_V} | jq -r '.assets[].name' | grep -E ${PACKAGE} | grep -E -v '\.md5$' | sort -V)"
export BRANCHES="$(wget -qO- https://raw.githubusercontent.com/DanITman/unraid-nvidia-vgpu-driver/master/nvidia_vgpu_versions | grep -v "UPDATED")"
export DL_URL="https://github.com/DanITman/unraid-nvidia-vgpu-driver/releases/download/${KERNEL_V}"
...
if [ "${SET_DRV_V}" == "latest" ]; then
  ...
  LAT_PRB_AVAIL="$(echo "$BRANCHES" | grep 'PRB' | cut -d '=' -f3 | sort -V)"
  if [ -z "$LAT_PRB_AVAIL" ]; then
    ...
    if [ -z "$(comm -12 <(echo "$DRIVER_AVAIL" | cut -d '-' -f3) <(echo "$LAT_PRB_AVAIL") | sort -V | tail -1)" ]; then
      if [ -z "${CUR_V}" ]; then
        echo
        echo "-----ERROR - ERROR - ERROR - ERROR - ERROR - ERROR - ERROR - ERROR - ERROR------"
        echo "---Can't get latest Nvidia driver version and found no installed local driver---"
        ...
        echo "---Can't find Nvidia vGPU Driver v${SET_DRV_V} for your Kernel v${KERNEL_V%%-*} falling back to latest Nvidia vGPU Driver v$(echo $LAT_PACKAGE | cut -d '-' -f3)---"
  ...
  LAT_NFB_AVAIL="$(echo "$BRANCHES" | grep 'NFB' | cut -d '=' -f3 | sort -V)"
  if [ -z "$LAT_NFB_AVAIL" ]; then
    ...
    if [ -z "$(comm -12 <(echo "$DRIVER_AVAIL" | cut -d '-' -f3) <(echo "$LAT_NFB_AVAIL") | sort -V | tail -1)" ]; then
      if [ -z "${CUR_V}" ]; then
        echo
        echo "-----ERROR - ERROR - ERROR - ERROR - ERROR - ERROR - ERROR - ERROR - ERROR------"
        echo "------Can't get Nvidia driver versions and found no installed local driver------"
